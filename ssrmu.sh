...
Set_config_user(){
	...
	stty erase '^H' && read -p "(默认: oogh):" ssr_user
	[[ -z "${ssr_user}" ]] && ssr_user="oogh"
...
Set_config_port(){
	...
	stty erase '^H' && read -p "(默认: 2016):" ssr_port
	[[ -z "$ssr_port" ]] && ssr_port="2016"
...
Set_config_password(){
	echo "请输入要设置的用户 密码"
	stty erase '^H' && read -p "(默认: oogh):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="oogh"
...
View_user_connection_info(){
 ...
 ${Green_font_prefix}2.${Font_color_suffix} 显示 IP+IP归属地 格式" && echo
	stty erase '^H' && read -p "(默认: 2):" ssr_connection_info
